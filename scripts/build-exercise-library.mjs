...
//
//  Output:
//    src/lib/exercises.js          — enriched exercises (same shape + new fields)
...
async function main() {
  ...
  // ─── 5. Build enrichment map for existing exercises ───
  const enrichMap = {};
  for (const m of matched) {
    enrichMap[m.talos.name] = {
  ...
  console.log(`   ${matched.length} enriched with instructions + images`);
...
function formatExerciseLine(tex, enrich) {
  ...
  if (enrich) {
    ...
    if (enrich.level)                   parts.push(`level: "${enrich.level}"`);
    if (enrich.force)                   parts.push(`force: "${enrich.force}"`);
    if (enrich.mechanic)                parts.push(`mechanic: "${enrich.mechanic}"`);
    if (enrich.instructions?.length)    parts.push(`instructions: ${JSON.stringify(enrich.instructions)}`);
    if (enrich.images?.length)          parts.push(`images: ${JSON.stringify(enrich.images)}`);
  }
  return `  { ${parts.join(", ")} },`;
}
...
function formatNewExerciseLine(nex) {
  ...
  if (nex.level)               parts.push(`level: "${nex.level}"`);
  if (nex.force)               parts.push(`force: "${nex.force}"`);
  if (nex.mechanic)            parts.push(`mechanic: "${nex.mechanic}"`);
  if (nex.instructions?.length) parts.push(`instructions: ${JSON.stringify(nex.instructions)}`);
  if (nex.images?.length)      parts.push(`images: ${JSON.stringify(nex.images)}`);
  return `  { ${parts.join(", ")} },`;
}
